...
const deleteOldFeedItems = `-- name: DeleteOldFeedItems :execresult
DELETE FROM feed_items WHERE scraped_at < datetime('now', '-' || ? || ' hours')
`

func (q *Queries) DeleteOldFeedItems(ctx context.Context, dollar_1 sql.NullString) (sql.Result, error) {
	return q.db.ExecContext(ctx, deleteOldFeedItems, dollar_1)
}
